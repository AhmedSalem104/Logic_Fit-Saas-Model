...
- **APIs:** `GET /gyms/{gymId}/members`; `POST /gyms/{gymId}/members`; `GET/PUT/DELETE /gyms/{gymId}/members/{memberId}`; `GET /gyms/{gymId}/members/{memberId}/timeline`; plus the explicitly contracted membership and attendance routes if those linked flows are authorized in the slice. Every route requires selected Gym context, backend permission enforcement, server validation, row-version handling where specified, audit, and no cross-Gym query.
